...
bool Resources::LoadConfig() {                     // read config from text file, returns false if failed, true otherwise
...
bool Resources::Init(const int & argc, char* argv[]){
    // init SDL
    if(!window->Init_SDL())
    {
        window->Close_SDL();
